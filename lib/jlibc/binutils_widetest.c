...
void test_hexstr2bits() {
    uint8_t n = 82;
    uint8_t hex_orders = n % NIBBLE == 0 ? n / NIBBLE : (n / NIBBLE) + 1;

    // Test hexstr2bits
    char g[] = "0x0308c0111011401440411";
    uint8_t gBits[129] = {};
    hexstr2bitsMSF(16, g, gBits, true);
    gBits[COUNT_OF(gBits)-1-n] = 1;
    printBits("      Poly converted", gBits, COUNT_OF(gBits), 0);
    
    char check[] = "0x09ea83f625023801fd612";
    uint8_t checkBits[128] = {};
    hexstr2bitsMSF(16, check, checkBits, false);
    printBits("     Check converted", checkBits, COUNT_OF(checkBits), 0);

    // Prep for test bits2hexstr
    uint8_t gBits_noIL1[128] = {};
    hexstr2bitsMSF(16, g, gBits_noIL1, false);
    printBits("         Poly no il1", gBits_noIL1, COUNT_OF(gBits_noIL1), 0);

    // Test bits2hexstr
    char g_out[hex_orders + 1]; // fit \0
    uint8_t gBits_cropped[n];
    bitSlice(COUNT_OF(gBits_noIL1)-n, n, gBits_noIL1, 0, gBits_cropped);
    printBits("           g cropped", gBits_cropped, COUNT_OF(gBits_cropped), 0);
    // bits2hexstrMSF(COUNT_OF(gBits_noIL1), gBits_noIL1, g_out);
    bits2hexstrMSF(n, gBits_cropped, g_out);
    printf("g no il1: %s\n", g_out);

    puts("");
    char check_out[hex_orders + 1]; // fit \0
    uint8_t checkBits_cropped[n];
    bitSlice(COUNT_OF(checkBits)-n, n, checkBits, 0, checkBits_cropped);
    printBits("      check cropped", checkBits_cropped, COUNT_OF(checkBits_cropped), 0);
    bits2hexstrMSF(n, checkBits_cropped, check_out);
    printf("   check: %s\n", check_out);
